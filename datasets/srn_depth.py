...
class SRNDepthDataset(SharedDataset):
    ...
    def load_depth(self, depth_path):
        ...
        depth = PILtoTorch(depth, (self.cfg.data.training_resolution, self.cfg.data.training_resolution))
        return depth.unsqueeze(0)  # Add channel dimension
            ...
            for i, cam_info in enumerate(cam_infos):
                ...
                if self.use_depth:
                    ...
                    if depth.dim() == 2:
                        ...
                        depth = depth.mean(dim=0, keepdim=True)  # Average across channels if there are multiple
                    elif depth.dim() == 4:
                        depth = depth.squeeze(0)  # Remove the extra dimension if it exists
                    
                    # Ensure depth has the same spatial dimensions as rgb
                    if depth.shape[-2:] != rgb.shape[-2:]:
                        depth = torch.nn.functional.interpolate(depth.unsqueeze(0), 
                                                                size=rgb.shape[-2:], 
                                                                mode='nearest').squeeze(0)
                    
                    # Ensure depth has only one channel
                    depth = depth[:1]  # Take only the first channel if there are multiple
                    
                    # Ensure both tensors have exactly 3 dimensions
                    assert rgb.dim() == 3 and depth.dim() == 3, f"Dimension mismatch: RGB {rgb.shape}, Depth {depth.shape}"
                    assert rgb.shape[1:] == depth.shape[1:], f"Spatial dimension mismatch: RGB {rgb.shape}, Depth {depth.shape}"
                    
                    combined = torch.cat([rgb, depth * self.depth_scale], dim=0)
                    assert combined.shape[0] == 4, f"Combined channels should be 4, but got {combined.shape[0]}"
                    self.all_rgbs[example_id].append(combined)
                else:
                    self.all_rgbs[example_id].append(rgb)
                
                # print(f"RGB shape: {rgb.shape}")
                # print(f"RGB min/max: {rgb.min().item()}, {rgb.max().item()}")
                # print(f"Depth shape: {depth.shape}")
                # print(f"Depth min/max: {depth.min().item()}, {depth.max().item()}")
                # print(f"Combined shape: {combined.shape}")
                # print(f"Combined min/max: {combined.min().item()}, {combined.max().item()}")
